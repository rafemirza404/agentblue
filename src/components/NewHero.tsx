...
const NewHero = () => {
  ...
  return (
    <section className="bg-[#EEF4FF] py-16 md:py-36">
      <div className="container mx-auto px-4 max-w-5xl flex flex-col items-center text-center">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 mb-4 animate-slide-up-subtle animate-delay-200">
          <Button
            onClick={openSophia}
            className="bg-[#4F7CFF] hover:bg-[#4F7CFF] text-white text-base md:text-lg font-semibold px-6 md:px-8 py-3 md:py-7 rounded-full transition-all duration-300 hover:scale-105 shadow-[0_0_20px_rgba(79,124,255,0.4)] hover:shadow-[0_0_30px_rgba(79,124,255,0.6)]"
          >
            Talk to Sophia <ArrowRight className="ml-2 h-4 w-4 md:h-5 md:w-5 inline" />
          </Button>

          <CalendarBooking
            trigger={
              <button className="flex items-center gap-2 text-gray-700 font-semibold text-base md:text-lg hover:text-black transition-colors duration-200 cursor-pointer">
                Book a call <ArrowRight className="h-4 w-4 md:h-5 md:w-5" />
              </button>
            }
          />
        </div>
